  ...
  while (it.hasNext()) {
    ...
    if (remove) {
      if (res.type == "cover" || res.type == "screenshot" ||
          res.type == "wheel" || res.type == "marquee" || res.type == "video") {
  ...
  for (const auto &resType : resTypeList) {
    ...
    if (reportFile.open(QIODevice::WriteOnly)) {
      ...
      printf("Report file could not be opened for writing, please check "
             "permissions of folder '%s', then try again...\n",
             reportsDir.absolutePath().toStdString().c_str());
      return;
    }
  }
...
QString Cache::getQuickId(const QFileInfo &info) {
  ...
  if (quickIds.contains(info.completeBaseName()) &&
      info.lastModified().toMSecsSinceEpoch() <=
          quickIds[info.completeBaseName()].first) {
