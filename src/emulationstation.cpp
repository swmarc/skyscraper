...
void EmulationStation::assembleList(QString &finalOutput,
  ...
  for (auto &entry : gameEntries) {
    ...
    if (entryInfo.isFile() && config->platform != "daphne") {
      ...
      // Check if path is exactly one subfolder beneath root platform folder
      // (has one more '/')
      if (entryAbsolutePath.count("/") == config->inputFolder.count("/") + 1) {
        QString extensions = Platform::getFormats(
            config->platform, config->extensions, config->addExtensions);
        // Check if the platform has both cue and bin extensions. Remove bin if
        // it does to avoid count() below to be 2 I thought about removing bin
        // extensions entirely from platform.cpp, but I assume I've added them
        // per user request at some point.
        if (extensions.contains("*.cue") && extensions.contains("*.bin")) {
          extensions.replace("*.bin", "");
          extensions = extensions.simplified();
        }
        // Check is subfolder has more roms than one, in which case we stick
        // with <game>
    ...
    finalOutput.append("  <" + entryType + " romname=\"" +
                       StrTools::xmlEscape(entry.baseName) + "\">\n");
    ...
    if (entry.coverFile.isEmpty()) {
      ...
      // The replace here IS supposed to be 'inputFolder' and not 'mediaFolder'
      // because we only want the path to be relative if '-o' hasn't been set.
      // So this will only make it relative if the path is equal to inputFolder
    ...
    if (entry.rating.isEmpty()) {
      ...
      finalOutput.append("    <rating>" + StrTools::xmlEscape(entry.rating) +
                         "</rating>\n");
    }
    if (entry.description.isEmpty()) {
    ...
    if (entry.players.isEmpty()) {
      ...
      finalOutput.append("    <players>" + StrTools::xmlEscape(entry.players) +
                         "</players>\n");
    }
    if (!entry.eSSortName.isEmpty()) {
    ...
    if (!entry.eSHidden.isEmpty()) {
      finalOutput.append("    <hidden>" + StrTools::xmlEscape(entry.eSHidden) +
                         "</hidden>\n");
    }
    if (!entry.eSLastPlayed.isEmpty()) {
    ...
    if (entry.eSKidGame.isEmpty()) {
      ...
      finalOutput.append("    <kidgame>" +
                         StrTools::xmlEscape(entry.eSKidGame) + "</kidgame>\n");
    }
...
QString EmulationStation::getReportsFolder() { return config->reportsFolder; }
