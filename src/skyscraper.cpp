...
void Skyscraper::run() {
...

  {
    if (config.unpack) {
      ...
      if (!decProc.readAllStandardOutput().contains("7z")) {
        printf("Couldn't find '7z' command. 7z is required by the '--flags "
               "unpack' flag. On Debian derivatives such as RetroPie you can "
  ...
  if (!config.cacheFolder.isEmpty()) {
    ...
    if (cache->createFolders(config.scraper)) {
      if (!cache->read() && config.scraper == "cache") {
        printf("No resources for this platform found in the resource cache. "
               "Please specify a scraping module with '-s' to gather some "
               "resources before trying to generate a game list. Check all "
               "available modules with '--help'. You can also run Skyscraper "
               "in simple mode by typing 'Skyscraper' and follow the "
               "instructions on screen.\n\n");
        exit(1);
      }
    } else {
      printf("Couldn't create cache folders, please check folder permissions "
             "and try again...\n");
      exit(1);
    }
  }
  ...
  if (!inputDir.exists()) {
    printf("Input folder '\033[1;32m%s\033[0m' doesn't exist or can't be seen "
           "by current user. Please check path and permissions.\n",
  ...
  if (!config.pretend && config.scraper == "cache" && !config.unattend &&
    ...
    printf("\033[1;34m'\033[0m\033[1;33m%s\033[0m\033[1;34m' already exists, "
           "do you want to overwrite it\033[0m (y/N)? ",
    ...
    if (gameListFile.open(QIODevice::Append)) {
      ...
      printf("\033[1;31mIt isn't! :(\nPlease check path and permissions and "
             "try again.\033[0m\n");
      exit(1);
    }
    printf("\n");
      ...
      if (!config.unattend && cliFiles.isEmpty()) {
        ...
        if (gameListFile.exists() && frontend->canSkip()) {
          if (config.unattendSkip) {
            ...
            printf("\033[1;34mDo you want to skip already existing game list "
                   "entries\033[0m (y/N)? ");
            getline(std::cin, userInput);
          }
  ...
  if (config.romLimit != -1 && totalFiles > config.romLimit) {
    printf("\n\033[1;33mRestriction overrun!\033[0m This scraping module only "
           "allows for scraping up to %d roms at a time. You can either supply "
           "a few rom filenames on command line, or make use of the "
  ...
  if (totalFiles > 0) {
    printf("Starting scraping run on \033[1;32m%d\033[0m files using "
           "\033[1;32m%d\033[0m threads.\nSit back, relax and let me do the "
           "work! :)\n\n",
  ...
  if (!folder.exists()) {
    ...
    if (create) {
      ...
      if (folder.mkpath(folder.absolutePath())) {
        ...
        printf("\033[1;32mFailed!\033[0m Please check path and permissions, "
               "now exiting...\n");
        exit(1);
      }
    } else {
...
void Skyscraper::entryReady(const GameEntry &entry, const QString &output,
  ...
  if (entry.found) {
    ...
    if (entry.searchMatch == 0) {
      ...
      skippedFile.write(", Closest match was '" + entry.title.toUtf8() +
                        "' at " + QByteArray::number(entry.searchMatch) +
                        "%\n");
    }
    skippedFile.close();
    if (config.skipped) {
  ...
  if (config.spaceCheck) {
    if (config.scraper == "cache" && !config.pretend &&
      ...
      printf("\033[1;31mYou have very little disk space left on the Skyscraper "
             "media export drive, please free up some space and try again. Now "
             "aborting...\033[0m\n\n");
      printf(
          "Note! You can disable this check by setting 'spaceCheck=\"false\"' "
          "in the '[main]' section of config.ini.\n\n");
      // By clearing the queue here we basically tell Skyscraper to stop and
      // quit nicely
      config.pretend = true;
      queue->clearAll();
               ...
               spaceLimit) {
      printf("\033[1;31mYou have very little disk space left on the Skyscraper "
             "resource cache drive, please free up some space and try again. "
             "Now aborting...\033[0m\n\n");
      printf(
          "Note! You can disable this check by setting 'spaceCheck=\"false\"' "
          "in the '[main]' section of config.ini.\n\n");
...
void Skyscraper::checkThreads() {
  ...
  if (!config.pretend && config.scraper == "cache") {
    ...
    printf(
        "\033[1;34m---- Resource gathering run completed! YAY! ----\033[0m\n");
    if (!config.cacheFolder.isEmpty()) {
      state = 1; // Ignore ctrl+c
      cache->write();
      state = 0;
...
void Skyscraper::loadConfig(const QCommandLineParser &parser) {
  ...
  copyFile("/usr/local/etc/skyscraper/config.ini.example",
           "config.ini.example");
  ...
  copyFile("/usr/local/etc/skyscraper/artwork.xml.example1",
           "artwork.xml.example1");
  copyFile("/usr/local/etc/skyscraper/artwork.xml.example2",
           "artwork.xml.example2");
  copyFile("/usr/local/etc/skyscraper/artwork.xml.example3",
           "artwork.xml.example3");
  copyFile("/usr/local/etc/skyscraper/artwork.xml.example4",
           "artwork.xml.example4");
  copyFile("/usr/local/etc/skyscraper/mameMap.csv", "mameMap.csv");
  copyFile("/usr/local/etc/skyscraper/tgdb_developers.json",
           "tgdb_developers.json");
  copyFile("/usr/local/etc/skyscraper/tgdb_publishers.json",
           "tgdb_publishers.json");
  copyFile("/usr/local/etc/skyscraper/resources/boxfront.png",
           "resources/boxfront.png");
  copyFile("/usr/local/etc/skyscraper/resources/boxside.png",
           "resources/boxside.png");
  copyFile("/usr/local/etc/skyscraper/docs/CACHE.md", "cache/README.md");
  copyFile("/usr/local/etc/skyscraper/cache/priorities.xml.example",
           "cache/priorities.xml.example");
  copyFile("/usr/local/etc/skyscraper/import/IMPORT.md", "import/README.md");
  copyFile("/usr/local/etc/skyscraper/import/definitions.dat.example1",
           "import/definitions.dat.example1");
  copyFile("/usr/local/etc/skyscraper/import/definitions.dat.example2",
  ...
  // Make sure we have a default config.ini file based on the config.ini.example
  // file
  ...
  copyFile("/usr/local/etc/skyscraper/resources/maskexample.png",
           "resources/maskexample.png", false);
  copyFile("/usr/local/etc/skyscraper/resources/frameexample.png",
           "resources/frameexample.png", false);
  copyFile("/usr/local/etc/skyscraper/resources/scanlines1.png",
           "resources/scanlines1.png", false);
  copyFile("/usr/local/etc/skyscraper/resources/scanlines2.png",
           "resources/scanlines2.png", false);
  // Copy one of the example definitions.dat files if none exists
  copyFile("/usr/local/etc/skyscraper/import/definitions.dat.example2",
           "import/definitions.dat", false);
  ...
  settings.endGroup();
  if (parser.isSet("f") &&
      (parser.value("f") == "emulationstation" ||
       parser.value("f") == "attractmode" || parser.value("f") == "pegasus")) {
  ...
  // config.ini entries
  if ((parser.isSet("p") &&
       Platform::getPlatforms().contains(parser.value("p"))) ||
  ...
  if (parser.isSet("s") &&
      (parser.value("s") == "openretro" || parser.value("s") == "thegamesdb" ||
       parser.value("s") == "arcadedb" || parser.value("s") == "igdb" ||
  ...
  if (parser.isSet("flags")) {
    if (parser.value("flags") == "help") {
      ...
      printf("Use comma-separated flags (eg. '--flags FLAG1,FLAG2') to enable "
             "multiple flags.\nThe following is a list of valid flags and what "
             "they do:\n");

      printf("  \033[1;33mforcefilename\033[0m: Use filename as game name "
             "instead of the returned game title when generating a game list. "
             "Consider using 'nameTemplate' config.ini option instead.\n");
      printf("  \033[1;33minteractive\033[0m: Always ask user to choose best "
             "returned result from the scraping modules.\n");
      printf("  \033[1;33mnobrackets\033[0m: Disables any [] and () tags in "
             "the frontend game titles. Consider using 'nameTemplate' "
             "config.ini option instead.\n");
      printf("  \033[1;33mnocovers\033[0m: Disable covers/boxart from being "
             "cached locally. Only do this if you do not plan to use the cover "
             "artwork in 'artwork.xml'\n");
      printf("  \033[1;33mnocropblack\033[0m: Disables cropping away black "
             "borders around screenshot resources when compositing the final "
             "gamelist artwork.\n");
      printf("  \033[1;33mnohints\033[0m: Disables the 'DID YOU KNOW:' hints "
             "when running Skyscraper.\n");
      ...
      printf("  \033[1;33mnoresize\033[0m: Disable resizing of artwork when "
             "saving it to the resource cache. Normally they are resized to "
             "save space.\n");
      printf("  \033[1;33mnoscreenshots\033[0m: Disable screenshots/snaps from "
             "being cached locally. Only do this if you do not plan to use the "
      ...
      printf("  \033[1;33mnowheels\033[0m: Disable wheels from being cached "
             "locally. Only do this if you do not plan to use the wheel "
             "artwork in 'artwork.xml'\n");
      printf(
          "  \033[1;33monlymissing\033[0m: Tells Skyscraper to skip all files "
          "which already have any data from any source in the cache.\n");
      printf("  \033[1;33mpretend\033[0m: Only relevant when generating a game "
             "list. It disables the game list generator and artwork compositor "
             "and only outputs the results of the potential game list "
             "generation to the terminal. Use it to check what and how the "
             "data will be combined from cached resources.\n");
      printf("  \033[1;33mrelative\033[0m: Forces all gamelist paths to be "
             "relative to rom location.\n");
      printf("  \033[1;33mskipexistingcovers\033[0m: When generating "
             "gamelists, skip processing covers that already exist in the "
             "media output folder.\n");
      printf("  \033[1;33mskipexistingmarquees\033[0m: When generating "
             "gamelists, skip processing marquees that already exist in the "
             "media output folder.\n");
      printf("  \033[1;33mskipexistingscreenshots\033[0m: When generating "
             "gamelists, skip processing screenshots that already exist in the "
             "media output folder.\n");
      printf("  \033[1;33mskipexistingvideos\033[0m: When generating "
             "gamelists, skip copying videos that already exist in the media "
             "output folder.\n");
      printf("  \033[1;33mskipexistingwheels\033[0m: When generating "
             "gamelists, skip processing wheels that already exist in the "
             "media output folder.\n");
      printf("  \033[1;33mskipped\033[0m: When generating a gamelist, also "
             "include games that do not have any cached data.\n");
      printf("  \033[1;33msymlink\033[0m: Forces cached videos to be symlinked "
             "to game list destination to save space. WARNING! Deleting or "
             "moving files from your cache can invalidate the links!\n");
      printf("  \033[1;33mtheinfront\033[0m: Forces Skyscraper to always try "
             "and move 'The' to the beginning of the game title when "
             "generating gamelists. By default 'The' will be moved to the end "
             "of the game titles.\n");
      printf("  \033[1;33munattend\033[0m: Skip initial questions when "
             "scraping. It will then always overwrite existing gamelist and "
             "not skip existing entries.\n");
      printf("  \033[1;33munattendskip\033[0m: Skip initial questions when "
             "scraping. It will then always overwrite existing gamelist and "
             "always skip existing entries.\n");
      printf("  \033[1;33munpack\033[0m: Unpacks and checksums the file inside "
             "7z or zip files instead of the compressed file itself. Be aware "
             "that this option requires '7z' to be installed on the system to "
             "work. Only relevant for 'screenscraper' scraping module.\n");
      printf("  \033[1;33mvideos\033[0m: Enables scraping and caching of "
             "videos for the scraping modules that support them. Beware, this "
      ...
      for (const auto &flag : flags) {
        if (flag == "forcefilename") {
          ...
          printf("Unknown flag '%s', please check '--flags help' for a list of "
                 "valid flags. Exiting...\n",
  ...
  if (parser.isSet("cache")) {
    ...
    if (config.cacheOptions == "refresh") {
      ...
      printf(
          ...
          "use '--includefrom' to edit files created with the '--cache report' "
      ...
      printf("  \033[1;33m--cache edit:new=<TYPE>\033[0m: Let's you batch add "
             ...
             "resources for just that one file, use '--includefrom' to edit "
             "files created with the '--cache report' option or use "
  ...
  if (parser.isSet("query")) {
    if (cliFiles.length() == 1) {
      ...
      printf("'--query' requires a single rom filename to be added at the end "
             "of the command-line. You either forgot to set one, or more than "
             "one was provided. Now quitting...\n");
      exit(1);
    }
  }
  ...
  if (artworkFile.open(QIODevice::ReadOnly)) {
    ...
    printf("Couldn't read artwork xml file '\033[1;32m%s\033[0m'. Please check "
           "file and permissions. Now exiting...\n",
...
void Skyscraper::copyFile(const QString &distro, const QString &current,
                          bool overwrite) {
...
void Skyscraper::doPrescrapeJobs() {
  ...
  if (config.scraper == "arcadedb" && config.threads != 1) {
    printf("\033[1;33mForcing 1 thread to accomodate limits in the ArcadeDB "
           "API\033[0m\n\n");
    config.threads =
        1; // Don't change! This limit was set by request from ArcadeDB
  } else if (config.scraper == "openretro" && config.threads != 1) {
    printf("\033[1;33mForcing 1 thread to accomodate limits in the OpenRetro "
           "API\033[0m\n\n");
    config.threads =
        1; // Don't change! This limit was set by request from OpenRetro
  } else if (config.scraper == "igdb") {
    if (config.threads > 4) {
      printf("\033[1;33mAdjusting to 4 threads to accomodate limits in the "
             "IGDB API\033[0m\n\n");
    ...
    if (config.user.isEmpty() || config.password.isEmpty()) {
      printf("The IGDB scraping module requires free user credentials to work. "
             "Read more about that here: "
             "'https://github.com/muldjord/skyscraper/blob/master/docs/"
    ...
    if (updateToken) {
      ...
      if (jsonObj.contains("access_token") && jsonObj.contains("expires_in") &&
        ...
        printf("\033[1;33mReceived invalid IGDB server response. This can be "
               "caused by server issues or maybe you entered your credentials "
               "incorrectly in the Skyscraper configuration. Read more about "
               "that here: "
               "'https://github.com/muldjord/skyscraper/blob/master/docs/"
    ...
    printf("\033[1;33mForcing 1 thread to accomodate limits in MobyGames "
    ...
    if (config.user.isEmpty() || config.password.isEmpty()) {
      if (config.threads > 1) {
        printf("\033[1;33mForcing 1 threads as this is the anonymous limit in "
               "the ScreenScraper scraping module. Sign up for an account at "
               "https://www.screenscraper.fr and support them to gain more "
               "threads. Then use the credentials with Skyscraper using the "
               "'-u user:password' command line option or by setting "
               "'userCreds=\"user:password\"' in "
               "'/home/USER/.skyscraper/config.ini'.\033[0m\n\n");
        config.threads =
            1; // Don't change! This limit was set by request from ScreenScraper
      }
    } else {
      printf("Fetching limits for user '\033[1;33m%s\033[0m', just a sec...\n",
             config.user.toStdString().c_str());
      netComm.request("https://www.screenscraper.fr/api2/"
                      "ssuserInfos.php?devid=muldjord&devpassword=" +
                      StrTools::unMagic("204;198;236;130;203;181;203;126;191;"
                                        "167;200;198;192;228;169;156") +
      ...
      if (jsonObj.isEmpty()) {
        if (netComm.getData().contains("Erreur de login")) {
          printf("\033[0;31mScreenScraper login error! Please verify that "
                 "you've entered your credentials correctly in "
                 "'/home/USER/.skyscraper/config.ini'. It needs to look "
                 "EXACTLY like this, but with your USER and "
                 "PASS:\033[0m\n\033[1;33m[screenscraper]\nuserCreds=\"USER:"
                 "PASS\"\033[0m\033[0;31m\nContinuing with unregistered user, "
                 "forcing 1 thread...\033[0m\n\n");
        } else {
          printf("\033[1;33mReceived invalid / empty ScreenScraper server "
                 "response, maybe their server is busy / overloaded. Forcing 1 "
                 "thread...\033[0m\n\n");
        }
        config.threads =
            1; // Don't change! This limit was set by request from ScreenScraper
        ...
        if (allowedThreads != 0) {
          if (config.threadsSet && config.threads <= allowedThreads) {
            printf("User is allowed %d threads, but user has set it manually, "
                   "so ignoring.\n\n",
                   allowedThreads);
          } else {
            config.threads = (allowedThreads <= 8 ? allowedThreads : 8);
            printf("Setting threads to \033[1;32m%d\033[0m as allowed for the "
