...
QString NameTools::getParNotes(QString baseName) {
  ...
  if (match.hasMatch()) {
    parNotes.append(
        "(" +
        match.captured(0).left(match.captured(0).indexOf("Disk")).trimmed() +
        " Disk)");
  }
  ...
  match =
      QRegularExpression("[_[]{1}(De|It|Pl|Fr|Es|Fi|Dk|Gr|Cz){1,10}[_\\]]{0,1}")
          .match(baseName);
  if (match.hasMatch()) {
    parNotes.append(
        "(" +
...
QString NameTools::getCacheId(const QFileInfo &info) {
  ...
  cacheId.addData(info.completeBaseName().toUtf8());
